package com.example.demoapp.Activities.authen;
...
import com.example.demoapp.Activities.CapLaiMatKhau;
                         ...
                         .addOnCompleteListener(new OnCompleteListener<String>() {
                             ...
                             public void onComplete(@NonNull Task<String> task) {
                                 if (task.isSuccessful()) {
                                     ...
                                     SharePreferencesManager manager = new SharePreferencesManager(
                                             SignIn.this);
                                     id = manager.getUserId();
//                            Log.d("tokennnnnn", id + " " + token);
                                     SharePreferencesManager manager1 = new SharePreferencesManager(
                                             SignIn.this);
...
//                            });
    ...
    public void processLogin() {
        loginBtn.setOnClickListener(v -> {
            String email = Objects.requireNonNull(txt_email.getText())
                                  .toString();
            String password = Objects.requireNonNull(txt_password.getText())
                                     .toString();
        ...
        ApiService.apiService.signIn(loginRequestDTO)
                             .enqueue(new Callback<BaseResponse<AuthenticationResponse>>() {
                                 ...
                                 public void onResponse(
                                         Call<BaseResponse<AuthenticationResponse>> call,
                                         Response<BaseResponse<AuthenticationResponse>> response
                                 ) {
                                     if (response.isSuccessful()) {
                                         Context context = SignIn.this;
                                         SharePreferencesManager sharePreferencesManager =
                                                 new SharePreferencesManager(context);
                                         sharePreferencesManager.saveToken(response.body()
                                                                                   .getData()
                                                                                   .getToken());
                                         // Đăng nhập thành công, chuyển đến MainActivity
                                         showSuccessDialog();
                                         redirectActivity(SignIn.this, MainActivity.class);
                                     } else {
                                         showErrorDialog();
                                     }
                                 }
